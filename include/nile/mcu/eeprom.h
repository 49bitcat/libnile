...
static inline int16_t nile_mcu_native_eeprom_read_sync(void __far* buffer, uint16_t offset, uint16_t length) {
    int16_t result;
    if ((result = nile_mcu_native_send_cmd(NILE_MCU_NATIVE_CMD(NILE_MCU_NATIVE_CMD_EEPROM_READ, length), &offset, 2)) < 0) return result;
    return nile_mcu_native_recv_cmd(buffer, length << 1);
}
...
static inline int16_t nile_mcu_native_eeprom_write_sync(const void __wf_cram* buffer, uint16_t offset, uint16_t length) {
    int16_t result;
    if ((result = nile_mcu_native_send_cmd(NILE_MCU_NATIVE_CMD(NILE_MCU_NATIVE_CMD_EEPROM_WRITE, length), &offset, 2)) < 0) return result;
    if (!nile_spi_tx_async_block(buffer, length << 1)) return NILE_MCU_NATIVE_ERROR_SPI;
